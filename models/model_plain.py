...
from models.lr_scheduler_dn import CosineAnnealingRestartCyclicLR
...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'l1sum':
            self.G_lossfn = nn.L1Loss(reduction='sum').to(self.device)
            self.G_lossfn_weight /= self.opt['datasets']['train']['dataloader_batch_size']
    ...
    def define_scheduler(self):
        if self.opt_train['G_scheduler_type'] == 'MultiStepLR':
            ...
            self.schedulers.append(lr_scheduler.CosineAnnealingLR(self.G_optimizer,
                                                            self.opt_train['G_scheduler_total_iters'],
                                                            self.opt_train['G_scheduler_eta_min']
                                                            ))
