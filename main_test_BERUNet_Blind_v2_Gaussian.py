...
def main():
    ...
    testset_name = 'Bsd68'               # set test set,  'Set12' | 'Bsd68' | 'Kodak24' | 'McMaster' | 'Urban100' 
                                         #| 'AI_TOD_150' | 'NWPU_RESISC45_50' | 'UCMerced_LandUse'
    ...
    if 'color' in task_current:
        model_path = os.path.join(model_pool, 'BERUNet', 'BERUNet_Blind_v2_gaussian_color.pth')
    else:
        model_path = os.path.join(model_pool, 'BERUNet', 'BERUNet_Blind_v2_gaussian_gray.pth')
    ...
    with torch.no_grad():
        for idx, img in enumerate(L_paths):
            ...
            # ------------------------------------
            if idx == 0:
                img_E = utils_model.test_impading_L(model, img_L, refield=8, refield_extension=0)
